...
export function standingPose(t: number, motion: number, seed: number, pose: BodyPose) {
  ...
  pose.shrug.left = -0.008 + 0.003 * breath;
  ...
  const headYaw = lerp(lerp(head.yaw + drift, 0, sip), toDog.yaw, 0.6 * dog);
  const headPitch = lerp(head.pitch + nod + 0.05 * think + 0.16 * tip, toDog.pitch, 0.6 * dog);
  const tilt = 0.035 - 0.13 * think + 0.06 * dog + noise(t * 0.15, seed + 5) * 0.04 * motion;
