...
import { createDogMaterials, type DogMaterials } from './materials';

const MaterialsContext = createContext<DogMaterials | null>(null);

/** Creates one material set for the dog and shares it with every part below. */
export function DogMaterialsProvider({ children }: { children: ReactNode }) {
  const materials = useDisposable(createDogMaterials);
