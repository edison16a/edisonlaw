...
/** How thick a line on the face is at its middle, and how much of its length each end takes to taper away. */
export interface FaceLineShape {
  radius: number;
  taper: number;
}

/**
 * A tube along the line of the mouth that swells from nothing at each corner, lying in the crease. Other
 * lines on the face, such as a shut eye's, pass their own `shape`.
 */
export function lipGeometry(line: Vec3[], shape: FaceLineShape = LIP) {
  const curve = new CatmullRomCurve3(
    line.map((point) => new Vector3(...point)),
    false,
    'centripetal',
  );
  const geometry = new TubeGeometry(curve, SEGMENTS.along, shape.radius, SEGMENTS.round, false);
  ...
  for (let ring = 0; ring <= SEGMENTS.along; ring++) {
    ...
    const end = Math.min(t, 1 - t) / shape.taper;
