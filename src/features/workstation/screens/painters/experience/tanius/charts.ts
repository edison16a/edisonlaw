...
const SMALL = { size: 9.5, family: 'mono', color: T.faint } as const;

function card(ctx: CanvasRenderingContext2D, rect: Rect, title: string, detail: string) {
  fillRound(ctx, rect.x, rect.y, rect.w, rect.h, 7, T.panel);
  strokeRound(ctx, rect.x, rect.y, rect.w, rect.h, 7, T.border);
  text(ctx, title, rect.x + 12, rect.y + 18, { size: 12.5, weight: 600, family: 'sans', color: T.text });
  text(ctx, detail, rect.x + rect.w - 12, rect.y + 18, { size: 11, family: 'sans', color: T.muted, align: 'right' });
}
...
export function candlePanel(ctx: CanvasRenderingContext2D, rect: Rect, live: number) {
  ...
  const plot = { x: rect.x + 38, y: rect.y + 36, w: rect.w - 92, h: rect.h - 50 };
  ...
  for (let i = 0; i <= 4; i++) {
    ...
    text(ctx, price.toFixed(0), plot.x + plot.w + 8, y, SMALL);
    text(ctx, vix.toFixed(1), plot.x - 7, y, { ...SMALL, color: '#8a6a2e', align: 'right' });
  }
  ...
  fillRound(ctx, plot.x + plot.w + 3, tagY - 8, 48, 16, 3, up ? T.up : T.down);
  text(ctx, last.close.toFixed(2), plot.x + plot.w + 27, tagY + 1, { size: 9.5, weight: 600, family: 'mono', color: '#ffffff', align: 'center' });

  fillRect(ctx, rect.x + 180, rect.y + 18, 11, 2, T.vix);
  text(ctx, 'VX1 front month, left axis', rect.x + 196, rect.y + 19, { size: 10.5, family: 'sans', color: T.muted });
}
...
export function correlationPanel(ctx: CanvasRenderingContext2D, rect: Rect) {
  ...
  const plot = { x: rect.x + 36, y: rect.y + 36, w: rect.w - 50, h: rect.h - 50 };
  const scale = { min: -1, max: 0.2 };
  gridLines(ctx, plot, 3, T.grid);
  [0, -0.5, -1].forEach((value) => text(ctx, value === 0 ? '0' : signed(value, 1), plot.x - 6, yOf(plot, scale, value), { ...SMALL, align: 'right' }));
  ctx.setLineDash([4, 4]);
...
export function scatterPanel(ctx: CanvasRenderingContext2D, rect: Rect) {
  ...
  const plot = { x: rect.x + 14, y: rect.y + 32, w: rect.w - 28, h: rect.h - 42 };
  ...
  RETURNS.forEach(([spy, vix]) => circle(ctx, px(spy), yOf(plot, yScale, vix), 2, T.blue));
  ctx.globalAlpha = 1;
  ctx.beginPath();
