...
export function panel(ctx: CanvasRenderingContext2D, grid: TerminalGrid, col: number, row: number, cols: number, rows: number, title: string, color: string) {
  drawCellBox(ctx, grid, col, row, cols, rows, T.border, { radius: grid.cellWidth * 0.8 });
  const label = ` ${title} `;
...
export function bar(ctx: CanvasRenderingContext2D, grid: TerminalGrid, col: number, row: number, cols: number, fraction: number, color: string) {
  ...
  const thickness = Math.round(grid.fontSize * 0.28);
  const y = Math.round(rowY(grid, row) + grid.cellHeight / 2 - thickness / 2);
  const width = cols * grid.cellWidth;
  ctx.fillStyle = T.track;
  ctx.fillRect(x, y, width, thickness);
  ctx.fillStyle = color;
  ctx.fillRect(x, y, Math.round(width * Math.min(1, fraction)), thickness);
}
...
export function spinner(ctx: CanvasRenderingContext2D, grid: TerminalGrid, col: number, row: number, frame: number, color: string) {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    ctx.fillStyle = color;
    ctx.beginPath();
    const orbit = grid.fontSize * 0.36;
    ctx.arc(cx + Math.cos(angle) * orbit, cy + Math.sin(angle) * orbit, grid.fontSize * 0.1, 0, Math.PI * 2);
    ctx.fill();
  }
  ctx.globalAlpha = 1;
}
