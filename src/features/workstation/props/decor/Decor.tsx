...
interface DecorProps {
  /** His coffee waits on the desk while he types. Standing, he holds it, so the desk goes without. */
  mugOnDesk: boolean;
}

/** The small things that make the room feel lived in, placed around the desk and on the wall. */
export function Decor({ mugOnDesk }: DecorProps) {
  return (
    <group>
      <WallShelf position={[-0.86, 1.74, 0]} />
      <CorkBoard position={[0.42, 1.86, 0]} />
      <FramedPicture position={[1.34, 1.52, 0]} />
      ...
      {mugOnDesk && <Mug position={[0.82, top, -0.02]} rotationY={-0.6} />}
