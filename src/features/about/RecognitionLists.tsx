...
function Row({ name, detail }: { name: string; detail: string }) {
  return (
    <li className="flex items-baseline justify-between gap-4 border-b border-grey-900 py-2.5 last:border-b-0">
      ...
      {detail && <span className="shrink-0 font-mono text-xs text-grey-400">{detail}</span>}
    </li>
  );
}
...
export function ActivitiesList() {
  ...
  return (
    <div className="flex flex-col gap-2">
      {shared && <p className="font-mono text-xs text-grey-400">Since {activities[0].since}</p>}
      <ul>
