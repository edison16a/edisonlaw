...
interface SkillCardProps {
  name: string;
  /** Plays no hover sound, for places that keep quiet, like a project's details. */
  silent?: boolean;
}

/** Identical small card: black face, thin grey border, the skill's logo in colour and its name. */
export function SkillCard({ name, silent = false }: SkillCardProps) {
  const icon = getSkillIcon(name);
  return (
    <li
      onPointerEnter={silent ? undefined : () => sound.play('hover')}
