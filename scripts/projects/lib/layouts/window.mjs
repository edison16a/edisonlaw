 ...
 * and a soft shadow, on a backdrop in the app's tones. The screenshot keeps its shape and is
 * centred with at least MARGIN clear on every side, so the backdrop fills the rest.
 */
import { CARD_HEIGHT, CARD_WIDTH } from '../encode.mjs';
import { MARGIN, fit, shell } from './base.mjs';

/**
 * `src` is the screenshot, `aspect` its width over height, `margin` the least space at its
 * edges, `radius` its corners and `background` any CSS background for the backdrop.
 */
export function appWindow({ src, aspect, margin = MARGIN, radius = 22, background, edge = '#fafbf8' }) {
  const { width, height } = fit(aspect, margin);
  const css = `
