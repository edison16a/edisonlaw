...
export function toPcm16(signal: Signal): Int16Array {
...
/** Encodes mono audio to a constant bitrate MP3 with lamejs. */
export async function encodeMp3(signal: Signal, kbps: number): Promise<Uint8Array> {
  ...
  for (let i = 0; i < pcm.length; i += FRAME) keep(encoder.encodeBuffer(pcm.subarray(i, i + FRAME)));
  keep(encoder.flush());
