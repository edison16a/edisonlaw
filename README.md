...
- The room is lit mostly by the setup: a fixed cool white area light in front of each monitor, a violet spill from the tower and violet washes on the wall and floor, and a faint moonlight. Each of these lights holds one colour and one strength, so the room never dims or shifts while you scroll and only the centre screen changes. One RGB clock cycles the hue of the glowing parts alone (tower fans and light strips), and their brightness is only partly evened out between hues. Bloom, AgX tone mapping, a vignette and a little grain finish it.
