...
- **Screenshot rows.** List up to four more pictures in a project's `screenshots` in `src/content/projects.ts`, add a recipe `scripts/projects/shots/<id>-2.mjs` and so on for each, and count them in `GALLERIES` in `scripts/projects/capture.mjs`.
